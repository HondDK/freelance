...
const OrdersPage = () => {
	...
	const itemsCategory = useFetch(
		"http://freelance_web:8001/freelance/orders/categories"
	);

	const itemsTags = useFetch(
		"http://freelance_web:8001/freelance/orders/tags"
	);

	const [items, setItems] = useState([]);
	const offset = (currentPage - 1) * itemsPerPage;
	const noSortedItems = useFetch(
		`http://freelance_web:8001/freelance/orders/orders?offset=${offset}&limit=${itemsPerPage}`
	);
	...
	function selectTag(e) {
		...
		fetch(`http://freelance_web:8001/freelance/orders/orders?tags=${tagUuids}`)
	...
	function selectCategory(e) {
		...
		fetch(
			`http://freelance_web:8001/freelance/orders/orders?categories=${categoryUuids}`
		)
			.then((response) => response.json())
			.then((data) => setItems(data));
	}
